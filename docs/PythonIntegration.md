...
4. opens a Sketch2 handle with `sk_new_handle()`

`sk_new_handle()` performs native runtime initialization automatically before the
handle is created.
...
- connects to the database root with `sk_new_handle()`
