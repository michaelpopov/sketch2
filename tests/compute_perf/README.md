...
Sketch2 compute path across different score functions (`cos`, `dot`, `l2`).
...
either the Highway engine or the NumKong engine.
...
- **Runtime Selection**: `--engine` is required. Perf runs intentionally use release artifacts only. Passing `--engine hwy` uses `REPO_ROOT/bin-hwy`, while `--engine nk` uses `REPO_ROOT/bin-nk`. The driver uses `Sketch2.compute_engine()` backed by the `sk_compute_engine()` API from the selected `libsketch2.so` to report the compiled engine and rejects mismatches.
...
```bash
make rel
./tests/compute_perf/driver.sh --engine hwy
```
...
```bash
make rel-nk
./tests/compute_perf/driver.sh --engine nk
```
