...
ensure_release_bin() {
    ...
    local -a required_paths=(
        ...
        "${REPO_ROOT}/bin/bench_compute"
    )

    for path in "${required_paths[@]}"; do
        if [[ ! -e "${path}" ]]; then
            echo "[driver] ERROR: required release artifact not found: ${path}" >&2
            echo "[driver] Build the release targets into ${REPO_ROOT}/bin before running compute perf tests." >&2
            return 1
        fi
    done
}

detect_compiled_engine() {
    local cache_path engine
    cache_path="${BUILD_DIR}/CMakeCache.txt"
    if [[ ! -f "${cache_path}" ]]; then
        echo "[driver] ERROR: build metadata not found: ${cache_path}" >&2
        echo "[driver] Configure the build tree in ${BUILD_DIR} before running compute perf tests." >&2
        return 1
    fi

    engine="$(awk -F= '/^SKETCH_COMPUTE_ENGINE:STRING=/{print $2; exit}' "${cache_path}")"
    if [[ "${engine}" == "highway" || "${engine}" == "numkong" ]]; then
        echo "${engine}"
        return 0
    fi

    echo "[driver] ERROR: unable to determine the compiled compute engine from ${cache_path}." >&2
    return 1
}

ensure_release_bin
readonly COMPUTE_PERF_COMPILED_ENGINE="$(detect_compiled_engine)"
...
# Legacy compatibility for runner/reporter config parsing: there is exactly one
# compiled top-level engine per build now.
export COMPUTE_PERF_TEST_ENGINES="${COMPUTE_PERF_COMPILED_ENGINE}"
...
echo "[driver]   compiled_engine=${COMPUTE_PERF_COMPILED_ENGINE}"
...
write_run_env() {
    {
        ...
        echo "COMPUTE_PERF_COMPILED_ENGINE=${COMPUTE_PERF_COMPILED_ENGINE}"
...
for dist in "${DIST_FUNCS[@]}"; do
    ...
    echo "[driver] benchmarks engine=${COMPUTE_PERF_COMPILED_ENGINE} dist=${dist}"
    unset SKETCH2_COMPUTE_ENGINE
    export COMPUTE_PERF_SINGLE_DIST="${dist}"
    set +e
    python3 runner.py 2>&1 | tee "${LOG_DIR}/runner_${COMPUTE_PERF_COMPILED_ENGINE}_${dist}.log"
    runner_rc=${PIPESTATUS[0]}
    set -e
    if [[ ${runner_rc} -ne 0 ]]; then
        echo "[driver] ERROR: runner.py failed for engine=${COMPUTE_PERF_COMPILED_ENGINE} dist=${dist} with exit code ${runner_rc}. See ${LOG_DIR}/runner_${COMPUTE_PERF_COMPILED_ENGINE}_${dist}.log"
        echo "[driver] diagnostics directory: ${DIAG_DIR}"
        if ls "${DIAG_DIR}"/diag_"${COMPUTE_PERF_COMPILED_ENGINE}"_*.json >/dev/null 2>&1; then
            echo "[driver] diagnostic state files:"
            ls "${DIAG_DIR}"/diag_"${COMPUTE_PERF_COMPILED_ENGINE}"_*.json
        fi
        if ls "${DIAG_DIR}"/repro_"${COMPUTE_PERF_COMPILED_ENGINE}"_*.sh >/dev/null 2>&1; then
            echo "[driver] repro scripts:"
            ls "${DIAG_DIR}"/repro_"${COMPUTE_PERF_COMPILED_ENGINE}"_*.sh
        fi
        if ls "${DIAG_DIR}"/repro_loop_"${COMPUTE_PERF_COMPILED_ENGINE}"_*.sh >/dev/null 2>&1; then
            echo "[driver] repro loop scripts:"
            ls "${DIAG_DIR}"/repro_loop_"${COMPUTE_PERF_COMPILED_ENGINE}"_*.sh
        fi
        exit 1
    fi
