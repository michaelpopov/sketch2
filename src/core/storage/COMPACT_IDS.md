...
`CompactIds` is the storage-layer representation for sorted active ids and
...
offsets from the first id. Once accumulation is finished, `CompactIds`
...
The payload encoding is selected by `CompactIds`:
...
At load time, `CompactIds::map()` reads `encoding` and forwards to the
...
8. Materialize `CompactIds` for active ids and deleted ids, then serialize both.

Important details:

- `CompactIdsAccumulator` is used while scanning ids.
- `CompactIds` is materialized once per section right before trailer output.
...
then parses the two `CompactIds` sections from the trailer.
...
- data-file merges pass an empty `CompactIds`, because the output `.data`
  file does not preserve tombstones
- delta-file merges build one `CompactIdsAccumulator` from the final delete list
  and materialize `CompactIds` when writing the trailer
...
4. After all vectors are written, call `write_ids_section()` to write:
   ...
   - alignment padding
   - active ids from `output_ids_`, materialized as `CompactIds`
   - deleted ids from the provided `CompactIds`
...
`CompactIds` section of the merged delta file.
...
In short, `CompactIds` is the sorted-id spine of the storage format:
