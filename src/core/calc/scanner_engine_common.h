...
inline Ret build_heap_with_kernels(const DatasetReader& dataset, DistFunc func,
        size_t count, const uint8_t* vec, const CalcKernels& kernels, DistHeap* heap,
    ...
    if (func == DistFunc::COS) {
        ...
        const FnPtrInvNormScore inv_score{kernels.dot, vec, dim, query_inv};
        const FnPtrQueryNormScore query_score{kernels.dist_with_query_norm, vec, dim, query_norm_sq};
        return build_dataset_heap_with_optional_stored_norms(
            dataset, count, inv_score, query_score, func, heap, bitset);
    }

    if (func == DistFunc::L2 && kernels.squared_norm && kernels.dot) {
        assert(kernels.dist);
        const double query_norm_sq = kernels.squared_norm(vec, dim);
...
inline Ret scanner_find_items_with_kernels(CalcEngine engine, const DatasetReader& dataset,
        size_t count, const uint8_t* vec, const CalcKernels& kernels, std::vector<DistItem>* result,
        const BitsetFilter* bitset) {
    ...
    CHECK(build_heap_with_kernels(dataset, func, count, vec, kernels, &heap, bitset));
    log_query(dataset.name(), func, dataset.type(), dataset.dim(), count, engine, timer.elapsed_ms());
