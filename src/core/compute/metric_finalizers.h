...
inline double l2_dist_from_squared_norms(double dot, double norm_a_sq, double norm_b_sq) {
    return std::max(0.0, norm_a_sq + norm_b_sq - (2.0 * dot));
}

// Cosine distance from squared norms. Includes the zero-vector contract shared
// by all Highway kernels: both zero → distance 0; one zero → distance 1.
inline double cos_dist_from_squared_norms(double dot, double norm_a_sq, double norm_b_sq) {
    ...
    const double cosine = std::clamp(dot / std::sqrt(norm_a_sq * norm_b_sq), -1.0, 1.0);
    return 1.0 - cosine;
}
