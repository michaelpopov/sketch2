...
#include "core/compute/compute_dot_norm_neon.h"
...
// ComputeCos_Neon owns cosine-specific distance entry points and calls the
// shared NEON dot/norm kernels from ComputeDotNorm_Neon when it needs query
// norms.
...
inline double ComputeCos_Neon::dist_f32(const uint8_t *a, const uint8_t *b, size_t dim) {
    return dist_f32_with_query_norm(a, b, dim, ComputeDotNorm_Neon::squared_norm_f32(b, dim));
}
...
inline double ComputeCos_Neon::dist_f16(const uint8_t *a, const uint8_t *b, size_t dim) {
    return dist_f16_with_query_norm(a, b, dim, ComputeDotNorm_Neon::squared_norm_f16(b, dim));
}
...
inline double ComputeCos_Neon::dist_f16_with_query_norm(const uint8_t *a, const uint8_t *b, size_t dim,
    ...
    double dot = static_cast<double>(vaddvq_f32(dot_acc0) + vaddvq_f32(dot_acc1));
    double norm_a = static_cast<double>(vaddvq_f32(norm_a_acc0) + vaddvq_f32(norm_a_acc1));
    for (; i < dim; ++i) {
        const double ai = static_cast<double>(va[i]);
        const double bi = static_cast<double>(vb[i]);
        dot += ai * bi;
...
inline double ComputeCos_Neon::dist_i16(const uint8_t *a, const uint8_t *b, size_t dim) {
    return dist_i16_with_query_norm(a, b, dim, ComputeDotNorm_Neon::squared_norm_i16(b, dim));
}
...
inline double ComputeCos_Neon::dist_i16_with_query_norm(const uint8_t *a, const uint8_t *b, size_t dim,
    ...
    for (; i < simd8_end; i += 8) {
        ...
        accumulate_mul_i32_as_i64(a_lo, b_lo, &dot_acc0, &dot_acc1);
        accumulate_mul_i32_as_i64(a_hi, b_hi, &dot_acc2, &dot_acc3);
        accumulate_mul_i32_as_i64(a_lo, a_lo, &norm_a_acc0, &norm_a_acc1);
        accumulate_mul_i32_as_i64(a_hi, a_hi, &norm_a_acc2, &norm_a_acc3);
    }
