...
#include "core/compute/compute_dot_norm_avx2.h"
...
// ComputeCos_AVX2 owns cosine-specific distance entry points and calls the
// shared AVX2 dot/norm kernels from ComputeDotNorm_AVX2 when it needs query
// norms.
...
SKETCH_AVX2_TARGET inline double ComputeCos_AVX2::dist_f32(const uint8_t *a, const uint8_t *b, size_t dim) {
    return dist_f32_with_query_norm(a, b, dim, ComputeDotNorm_AVX2::squared_norm_f32(b, dim));
}
...
SKETCH_AVX2_TARGET inline double ComputeCos_AVX2::dist_f16(const uint8_t *a, const uint8_t *b, size_t dim) {
    return dist_f16_with_query_norm(a, b, dim, ComputeDotNorm_AVX2::squared_norm_f16(b, dim));
}
...
SKETCH_AVX2_TARGET inline double ComputeCos_AVX2::dist_i16(const uint8_t *a, const uint8_t *b, size_t dim) {
    return dist_i16_with_query_norm(a, b, dim, ComputeDotNorm_AVX2::squared_norm_i16(b, dim));
}
