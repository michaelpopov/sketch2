...
inline double ComputeDotNorm_Neon::squared_norm_f32(const uint8_t *a, size_t dim) {
    ...
    double norm = static_cast<double>(vaddvq_f32(norm_acc0) + vaddvq_f32(norm_acc1));
    for (; i < dim; ++i) {
...
inline double ComputeDotNorm_Neon::squared_norm_i16(const uint8_t *a, size_t dim) {
    ...
    for (; i < simd8_end; i += 8) {
        ...
        accumulate_mul_i32_as_i64(a_lo, a_lo, &norm_acc0, &norm_acc1);
        accumulate_mul_i32_as_i64(a_hi, a_hi, &norm_acc2, &norm_acc3);
    }
