// Implements AVX2-optimized shared dot-product and squared-norm kernels.
...
// ComputeDotNorm_AVX2 owns the AVX2-specialized shared dot/norm kernels used
// by both cosine and L2 scan paths.
class ComputeDotNorm_AVX2 {
public:
...
SKETCH_AVX2_TARGET inline double ComputeDotNorm_AVX2::squared_norm_i16(const uint8_t *a, size_t dim) {
...
SKETCH_AVX2_TARGET inline double ComputeDotNorm_AVX2::dot_i16(const uint8_t *a, const uint8_t *b, size_t dim) {
    const int16_t *va = reinterpret_cast<const int16_t *>(a);
