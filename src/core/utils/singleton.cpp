...
Singleton::Singleton()
        : allowlist_spill_dir_(std::filesystem::temp_directory_path()) {}
...
size_t Singleton::allowlist_spill_threshold_bytes() const {
    return allowlist_spill_threshold_bytes_;
}

const std::filesystem::path& Singleton::allowlist_spill_dir() const {
    return allowlist_spill_dir_;
}
...
bool Singleton::collect_config_values_(const std::string* path, ConfigValues* values) {
    ...
    if (!config_path.empty()) {
        ...
        if (ret.code() == 0) {
            ...
            merged.allowlist_spill_threshold_bytes =
                reader.get_str("allowlist.spill_threshold_bytes", "");
            merged.allowlist_spill_dir = reader.get_str("allowlist.spill_dir", "");
        } else {
    ...
    const char* env_allowlist_spill_threshold_bytes =
        std::getenv("SKETCH2_ALLOWLIST_SPILL_THRESHOLD_BYTES");
    if (env_allowlist_spill_threshold_bytes != nullptr
            && env_allowlist_spill_threshold_bytes[0] != '\0') {
        LOG_INFO << "Allowlist spill threshold is set in env var: "
                 << env_allowlist_spill_threshold_bytes;
        merged.allowlist_spill_threshold_bytes = env_allowlist_spill_threshold_bytes;
    }

    const char* env_allowlist_spill_dir = std::getenv("SKETCH2_ALLOWLIST_SPILL_DIR");
    if (env_allowlist_spill_dir != nullptr && env_allowlist_spill_dir[0] != '\0') {
        LOG_INFO << "Allowlist spill directory is set in env var: " << env_allowlist_spill_dir;
        merged.allowlist_spill_dir = env_allowlist_spill_dir;
    }
...
bool Singleton::apply_config_values_(const ConfigValues& values, bool allow_defaults) {
    ...
    if (!values.allowlist_spill_threshold_bytes.empty()) {
        applied = apply_allowlist_spill_threshold_bytes_(
            values.allowlist_spill_threshold_bytes) || applied;
    }

    if (!values.allowlist_spill_dir.empty()) {
        applied = apply_allowlist_spill_dir_(values.allowlist_spill_dir) || applied;
    }
...
bool Singleton::apply_allowlist_spill_threshold_bytes_(const std::string& size) {
    if (size.empty()) {
        return false;
    }
    ...
    try {
        ...
        allowlist_spill_threshold_bytes_ = static_cast<size_t>(parsed);
    } catch (const std::exception&) {
        return false;
    }
...
bool Singleton::apply_allowlist_spill_dir_(const std::string& path) {
    if (path.empty()) {
        return false;
    }

    allowlist_spill_dir_ = path;
    return true;
}
